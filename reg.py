...
def main_worker(gpu, ngpus_per_node, args):
    ...
    training_set, validation_set, test_set = random_split(train_dataset, [num_training, num_val, num_test])


    train_loader = DataListLoader(training_set, batch_size=args.batch_size, shuffle=(train_sampler is None),num_workers=args.workers, pin_memory=True, sampler=train_sampler, drop_last=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not os.path.exists('./results/CSSL-Reg/'+args.dataset+'/'+str(args.batch_size)):
            os.makedirs('./results/CSSL-Reg/'+args.dataset+'/'+str(args.batch_size))
        if (epoch+1)%1==0:
            save_checkpoint({
                'epoch': epoch + 1,
                'arch': 'HGP-SL',
            ...
            }, is_best=False, filename='./results/CSSL-Reg/'+args.dataset+str(args.batch_size)+'/checkpoint_{:05d}.pth.tar'.format(epoch))

        if patience_cnt == args.patience or epoch == args.epochs - 1:
            model.load_state_dict(torch.load('./results/CSSL-Reg/'+args.dataset+str(args.batch_size)+'/checkpoint_{:05d}.pth.tar'.format(best_model))['state_dict'])
